...
class QUserWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        palette = QPalette()
        palette.setBrush(QPalette.Window, QBrush(QPixmap('./colorize/background/user/user2.jpg')))
    ...
    def displayImageToView2(self, method, ckpt):
        ...
        with open('colorize/method_images/' + str(method) + '.jpg', 'rb') as f:
            img = f.read()
        with open("colorize/CycleGAN/datasets/apple2orange/testB/1.jpg", 'wb') as f:
            f.write(img)

        with open(self.filename, 'rb') as f:
            img = f.read()
        with open("colorize/CycleGAN/datasets/apple2orange/testA/1.jpg", 'wb') as f:
            f.write(img)
        os.system("python colorize\\CycleGAN\\test.py --dataroot colorize\\CycleGAN\\datasets\\apple2orange --name " + ckpt + " --model cycle_gan --gpu_ids -1")
    ...
    def open_dataset(self):
        curPath = QDir.currentPath() + "//colorize//dataset"
    ...
    def processed(self):
        curPath = QDir.currentPath() + "/colorize/saved"
    ...
    def on_btnSavePic_clicked(self):
        ...
        curPath = QDir.currentPath() + "//colorize//saved/" + filename
        print(curPath)
